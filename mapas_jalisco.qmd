...
Este conjunto de datos para el estado de Nayarit incluye: - Límites estatales (archivos `18ent.*`) - Límites municipales (archivos `18mun.*`) - Otras divisiones territoriales
...
Primero, leemos el archivo shapefile para Nayarit:

```{r entidades}
# Nota: Asegúrate de que todos los archivos (shp, shx, dbf, prj, cpg) estén en el mismo directorio: data

Nayarit_estado <- st_read("data/18ent.shp", options = "ENCODING=LATIN1", quiet = TRUE)
```

### Visualización del estado de Nayarit

```{r mapa_Nayarit_estado}
# Crear mapa básico de Nayarit
mapa_Nayarit <- ggplot(Nayarit_estado) +
...
# Explorar los datos
cat("Número de municipios en Nayarit:", nrow(municipios_Nayarit), "\n")
cat("Primeros 10 municipios:\n")
print(municipios_Nayarit$NOMGEO[1:10])
```
...
municipios_Nayarit <- municipios_Nayarit %>%
  mutate(
...
mapa_destacados <- ggplot(municipios_Nayarit) +
  ...
  labs(
    title = "Municipios Importantes de Nayarit",
...
municipios_grandes <- municipios_Nayarit |> 
  st_drop_geometry() |> 
  arrange(desc(area_km2)) |> 
  select(NOMGEO, area_km2) |> 
  head(10)

print("Los 10 municipios más grandes de Nayarit:")
...
mapa_areas <- ggplot(municipios_Nayarit) +
  ...
  labs(
    title = "Área de los Municipios de Nayarit",
